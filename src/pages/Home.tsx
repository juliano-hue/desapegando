        ...
        <section className="rounded-3xl border border-white/10 bg-white/5 p-5 md:p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button variant="subtle" onClick={() => setShowFilters((v) => !v)}>
                <SlidersHorizontal className="h-4 w-4" />
                Filtros
              </Button>
              <Button onClick={() => void loadListings(1)}>Atualizar</Button>
            </div>
          </div>
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-[1fr_auto] md:items-center">
            ...
            <Button variant="primary" onClick={() => void loadListings(1)}>
              Buscar
            </Button>
          </div>
          ...
          {showFilters ? (
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <div className="rounded-2xl border border-white/10 bg-slate-950/30 p-4">
                <div className="text-xs font-semibold text-slate-200">Categoria</div>
                <div className="mt-3 flex flex-wrap gap-2">
                  <button
                    ...
                    className={
                      categoryId === null
                        ? 'rounded-full bg-emerald-300/18 px-3 py-1 text-xs font-semibold text-emerald-200'
                        : 'rounded-full bg-white/6 px-3 py-1 text-xs text-slate-200 hover:bg-white/10'
                    }
                  >
                    Todas
                  </button>
              ...
              <div className="rounded-2xl border border-white/10 bg-slate-950/30 p-4">
                <div className="text-xs font-semibold text-slate-200">Subcategoria</div>
                <div className="mt-3 flex flex-wrap gap-2">
                  <button
                    onClick={() => setSubCategoryId(null)}
                    className={
                      subCategoryId === null
                        ? 'rounded-full bg-cyan-300/14 px-3 py-1 text-xs font-semibold text-cyan-200'
                        : 'rounded-full bg-white/6 px-3 py-1 text-xs text-slate-200 hover:bg-white/10'
                    }
                  >
                    Todas
                ...
                {!selectedCategory ? (
                  <div className="mt-3 text-xs text-slate-400">Escolha uma categoria para ver subcategorias.</div>
                ) : null}
              </div>
            </div>
          ) : null}
